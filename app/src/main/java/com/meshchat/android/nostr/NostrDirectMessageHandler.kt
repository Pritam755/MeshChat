package com.meshchat.android.nostr
...
import com.meshchat.android.services.SeenMessageStore
import com.meshchat.android.ui.ChatState
import com.meshchat.android.ui.PrivateChatManager
import com.meshchat.android.ui.PrivateMessageOrigin
...
class NostrDirectMessageHandler(
    ...
    private val dataManager: com.meshchat.android.ui.DataManager,
    ...
    fun onGiftWrap(giftWrap: NostrEvent, geohash: String, identity: NostrIdentity) {
        scope.launch {
            try {
                ...
                if (packet.type != com.meshchat.android.protocol.MessageType.NOISE_ENCRYPTED.value) return@launch
                ...
                com.meshchat.android.nostr.GeohashAliasRegistry.put(convKey, senderPubkey)
        ...
        when (payload.type) {
            ...
            NoisePayloadType.FILE_TRANSFER -> {
                ...
                if (file != null) {
                    ...
                    val savedPath = com.meshchat.android.features.file.FileUtils.saveIncomingFile(application, file)
                    val message = BitchatMessage(
                        id = uniqueMsgId,
                        sender = senderNickname,
                        content = savedPath,
                    ...
                    if (!admitted) {
                        com.meshchat.android.features.file.FileUtils.deleteStoredMediaPaths(
                            application,
                            listOf(savedPath)
                        )
                    }
