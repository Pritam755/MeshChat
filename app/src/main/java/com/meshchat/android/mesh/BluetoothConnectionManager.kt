...
class BluetoothConnectionManager(
    ...
    init {
        ...
        try {
            val dbg = com.meshchat.android.ui.debug.DebugSettingsManager.getInstance()
    ...
    fun startServices(): Boolean {
        ...
        // }
            ...
            connectionScope.launch {
                ...
                Log.i(TAG, "[STAGE 3: ROLE INITIALIZATION] Operating roles configured: GattServer (Peripheral)=$startServer, GattClient (Central)=$startClient")

                if (startServer) {
                    if (!serverManager.start()) {
                        Log.e(TAG, "[STAGE 3: ROLE INITIALIZATION] ❌ Failed to start server manager (Peripheral role)")
                ...
                if (startClient) {
                    if (!clientManager.start()) {
                        Log.e(TAG, "[STAGE 3: ROLE INITIALIZATION] ❌ Failed to start client manager (Central role)")
                        this@BluetoothConnectionManager.isActive = false
                        return@launch
                    }
                } else {
                ...
                Log.i(TAG, "[STAGE 3: BLE READY] ✅ Bluetooth mesh services started successfully (Dual Role Active)")
            }
