...
class BluetoothGattServerManager(
    ...
    private fun isServerRoleEnabled(): Boolean {
        return isBleTransportEnabled() &&
            (try { com.meshchat.android.ui.debug.DebugSettingsManager.getInstance().gattServerEnabled.value } catch (_: Exception) { true })
    }
    ...
    private fun startAdvertising() {
        ...
        permissionManager.logPermissionAudit(TAG, "GATT Server / Advertising Start")

        // Guard conditions – never throw here to avoid crashing the app from a background coroutine
        if (!permissionManager.hasAdvertisePermissions()) {
            Log.w(TAG, "[STAGE 1: BLE PERMISSION AUDIT] ❌ Not starting advertising: missing Bluetooth advertise/connect permissions")
            return
        }
        if (bluetoothAdapter == null) {
            Log.w(TAG, "[STAGE 2: BLE ADAPTER CHECK] ❌ Not starting advertising: bluetoothAdapter is null")
            return
        }
        if (bluetoothAdapter.isEnabled != true) {
            Log.w(TAG, "[STAGE 2: BLE ADAPTER CHECK] ❌ Not starting advertising: Bluetooth adapter is DISABLED (OFF)")
            return
        }
        Log.i(TAG, "[STAGE 2: BLE ADAPTER CHECK] ✅ Bluetooth adapter is ENABLED (ON), state=${bluetoothAdapter.state}")
        ...
        Log.i(TAG, "[STAGE 3: BLE ADVERTISE START] Initiating advertisement | Service UUID: ${AppConstants.Mesh.Gatt.SERVICE_UUID} | PeerID prefix: ${myPeerID.take(16)}")
        ...
        advertiseCallback = object : AdvertiseCallback() {
            override fun onStartSuccess(settingsInEffect: AdvertiseSettings) {
                ...
                Log.i(TAG, "[STAGE 4: BLE ADVERTISE RESULT] ✅ Advertising STARTED successfully (power mode: $mode)")
            }

            override fun onStartFailure(errorCode: Int) {
                Log.e(TAG, "[STAGE 4: BLE ADVERTISE RESULT] ❌ Advertising FAILED with error code: $errorCode")
        ...
        try {
            ...
            Log.e(TAG, "[STAGE 3: BLE ADVERTISE START] ❌ SecurityException starting advertising (missing permission?): ${se.message}")
        } catch (e: Exception) {
            Log.e(TAG, "[STAGE 3: BLE ADVERTISE START] ❌ Exception starting advertising: ${e.message}")
        }
    }
