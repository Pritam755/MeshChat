...
import com.meshchat.android.model.PrivateMessagePacket
...
class MeshCore(
    ...
    private fun setupDelegates() {
        peerManager.delegate = object : PeerManagerDelegate {
            override fun onPeerListUpdated(peerIDs: List<String>) {
                try { com.meshchat.android.services.AppStateStore.setTransportPeers(transport.id, peerIDs) } catch (_: Exception) { }
        ...
        messageHandler.delegate = object : MessageHandlerDelegate {
            ...
            override fun updatePeerInfoFromVerifiedAnnouncement(
                ...
                isVerified: Boolean,
                capabilities: com.meshchat.android.model.PeerCapabilities?
            ): Boolean {
            ...
            override fun onDeliveryAckReceived(messageID: String, peerID: String) {
                try {
                    com.meshchat.android.services.AppStateStore.updatePrivateMessageStatus(
                        messageID,
                        com.meshchat.android.model.DeliveryStatus.Delivered(peerID, java.util.Date())
                    )
                } catch (_: Exception) { }
            ...
            override fun onReadReceiptReceived(messageID: String, peerID: String) {
                try {
                    com.meshchat.android.services.AppStateStore.updatePrivateMessageStatus(
                        messageID,
                        com.meshchat.android.model.DeliveryStatus.Read(peerID, java.util.Date())
                    )
                } catch (_: Exception) { }
    ...
    fun sendVerifyChallenge(peerID: String, noiseKeyHex: String, nonceA: ByteArray) {
        val payload = NoisePayload(
            ...
            data = com.meshchat.android.services.VerificationService.buildVerifyChallenge(noiseKeyHex, nonceA)
        )
        sendNoisePayloadToPeer(payload, peerID)
    }

    fun sendVerifyResponse(peerID: String, noiseKeyHex: String, nonceA: ByteArray) {
        val tlv = com.meshchat.android.services.VerificationService.buildVerifyResponse(noiseKeyHex, nonceA) ?: return
        val payload = NoisePayload(
    ...
    private fun buildAnnouncementPayload(announcement: IdentityAnnouncement, nickname: String): ByteArray? {
        ...
        try {
            if (directPeersForGossip.isNotEmpty()) {
                tlvPayload += com.meshchat.android.services.meshgraph.GossipTLV.encodeNeighbors(directPeersForGossip)
            }
    ...
    private fun getDirectPeerIDsForGossip(): List<String> {
        return try {
            ...
            try { com.meshchat.android.services.AppStateStore.setTransportDirectPeers(transport.id, localDirect) } catch (_: Exception) { }
            val union = try {
                com.meshchat.android.services.AppStateStore.getDirectPeers().ifEmpty { localDirect }
