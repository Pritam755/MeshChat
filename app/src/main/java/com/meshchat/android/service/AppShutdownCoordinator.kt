...
object AppShutdownCoordinator {
    ...
    fun requestFullShutdownAndKill(
        ...
        val job = scope.launch {
            ...
            try {
                val intent = android.content.Intent(com.meshchat.android.util.AppConstants.UI.ACTION_FORCE_FINISH)
                    .setPackage(app.packageName)
                app.sendBroadcast(intent, com.meshchat.android.util.AppConstants.UI.PERMISSION_FORCE_FINISH)
            } catch (_: Exception) { }

            // Stop mesh (best-effort)
            try { mesh?.stopServices() } catch (_: Exception) { }
            try { com.meshchat.android.nostr.NostrRelayManager.shared.disconnect() } catch (_: Exception) { }
            try { com.meshchat.android.mesh.PowerManager.getInstance(app).shutdown() } catch (_: Exception) { }
            ...
            try { com.meshchat.android.services.AppStateStore.clear() } catch (_: Exception) { }
