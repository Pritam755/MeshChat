...
import com.meshchat.android.ui.NotificationTextUtils
...
class MeshDelegateHandler(
    ...
    private suspend fun processPeerUpdate(mergedPeers: List<String>) {
        ...
        runCatching { com.meshchat.android.services.MessageRouter.tryGetInstance()?.onPeersUpdated(mergedPeers) }

        // Clean up channel members who disconnected
        channelManager.cleanupDisconnectedMembers(mergedPeers, getMyPeerID())

        runCatching { com.meshchat.android.services.AppStateStore.canonicalizePrivateChats() }
        ...
        state.getSelectedPrivateChatPeerValue()?.let { currentPeer ->
            ...
            if (canonical != currentPeer) {
                com.meshchat.android.services.ConversationAliasResolver.unifyChatsIntoPeer(
                    state = state,
    ...
    private fun unifyChatsIntoPeer(targetPeerID: String, keysToMerge: List<String>) {
        com.meshchat.android.services.ConversationAliasResolver.unifyChatsIntoPeer(state, targetPeerID, keysToMerge)
    }
        ...
        if (shouldSendReadReceipt) {
            ...
            try {
                val meshPeerID = ContactDirectory.resolve(senderConversationID).meshPeerID
                    ?: senderPeerID.takeIf {
                        com.meshchat.android.services.ContactIdentityResolver.isMeshPeerId(it)
                    }
                if (meshPeerID != null &&
     ...
     */
    fun getPeerInfo(peerID: String): com.meshchat.android.mesh.PeerInfo? {
