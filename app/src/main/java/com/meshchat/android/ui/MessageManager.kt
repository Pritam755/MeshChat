...
class MessageManager(private val state: ChatState) {
    ...
    private val MESSAGE_DEDUP_TIMEOUT = com.meshchat.android.util.AppConstants.UI.MESSAGE_DEDUP_TIMEOUT_MS // 30 seconds
    private val SYSTEM_EVENT_DEDUP_TIMEOUT = com.meshchat.android.util.AppConstants.UI.SYSTEM_EVENT_DEDUP_TIMEOUT_MS // 5 seconds
    ...
    fun addMessage(message: BitchatMessage) {
        ...
        try { com.meshchat.android.services.AppStateStore.addPublicMessage(message) } catch (_: Exception) { }
    }
    ...
    fun addChannelMessage(channel: String, message: BitchatMessage) {
        ...
        try { com.meshchat.android.services.AppStateStore.addChannelMessage(channel, message) } catch (_: Exception) { }
        ...
        val viewingGeohashChannel = try {
            if (channel.startsWith("geo:")) {
                ...
                selected is com.meshchat.android.geohash.ChannelID.Location && selected.channel.geohash.equals(geo, ignoreCase = true)
            } else false
    ...
    fun addPrivateMessage(peerID: String, message: BitchatMessage) {
        ...
        val accepted = try {
            com.meshchat.android.services.AppStateStore.addPrivateMessage(
                conversationID,
                message
            )
        } catch (_: Exception) {
    ...
    suspend fun addPrivateMessageDurably(
        ...
        val conversationID = ContactDirectory.canonicalConversationId(peerID)
        val accepted = try {
            com.meshchat.android.services.AppStateStore.addPrivateMessageDurably(
    ...
    fun clearPrivateMessages(peerID: String) {
        val conversationID = ContactDirectory.canonicalConversationId(peerID)
        com.meshchat.android.services.AppStateStore.deletePrivateConversation(conversationID)
    ...
    fun updateMessageDeliveryStatus(messageID: String, status: DeliveryStatus) {
        ...
        if (updated) {
            ...
            try { com.meshchat.android.services.AppStateStore.updatePrivateMessageStatus(messageID, status) } catch (_: Exception) { }
        }
    ...
    fun removeMessageById(messageID: String) {
        ...
        run {
            ...
            if (changed) {
                ...
                com.meshchat.android.services.AppStateStore.removePrivateMessage(messageID)
            }
        }
        // Channels
        run {
