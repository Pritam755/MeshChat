package com.meshchat.android.protocol
...
data class BitchatPacket(
    ...
    fun toBinaryDataForSigning(): ByteArray? {
        ...
        val unsignedPacket = BitchatPacket(
            ...
            route = route,
            ttl = com.meshchat.android.util.AppConstants.SYNC_TTL_HOPS // Use fixed TTL=0 for signing to ensure relay compatibility
        )
...
object BinaryProtocol {
    ...
    fun encode(packet: BitchatPacket, padding: Boolean = true): ByteArray? {
        try {
            val maxPayloadLength = com.meshchat.android.util.AppConstants.Protocol.MAX_PAYLOAD_LENGTH
    ...
    private fun decodeCore(
        ...
        try {
            ...
            if (payloadLength > com.meshchat.android.util.AppConstants.Protocol.MAX_PAYLOAD_LENGTH.toUInt()) {
                Log.w("BinaryProtocol", "Payload length ${payloadLength} exceeds maximum allowed (${com.meshchat.android.util.AppConstants.Protocol.MAX_PAYLOAD_LENGTH})")
                return null
            }
            ...
            val payload = if (isCompressed) {
                ...
                val maxExpandedSize = com.meshchat.android.util.AppConstants.Protocol.MAX_PAYLOAD_LENGTH
