package com.meshchat.watch.mesh
...
import com.meshchat.android.mesh.BluetoothConnectionManager
import com.meshchat.android.mesh.BluetoothConnectionManagerDelegate
...
import com.meshchat.android.services.AppStateStore
...
class WearMeshService private constructor(private val context: Context) {
    ...
    var onPrivateMessage: ((com.meshchat.android.model.BitchatMessage) -> Unit)? = null
    ...
    private fun handleMessageReceived(
        message: com.meshchat.android.model.BitchatMessage
    ): Boolean = try {
        when {
            message.isPrivate -> {
     ...
     */
    fun sendFilePrivateEncrypted(recipientPeerID: String, file: com.meshchat.android.model.BitchatFilePacket) {
        serviceScope.launch {
            ...
            val transferId = com.meshchat.android.mesh.MeshPacketUtils.sha256Hex(
            ...
            while (System.currentTimeMillis() < prepDeadline) {
                when (val prep = meshCore.prepareFilePrivate(recipientPeerID, file, transferId, allowLegacyFallback = false)) {
                    is com.meshchat.android.mesh.PrivateMediaPreparation.Ready -> {
                        prep.transfer.commit()
                        return@launch
                    }
                    com.meshchat.android.mesh.PrivateMediaPreparation.AwaitingPeerState,
                    com.meshchat.android.mesh.PrivateMediaPreparation.NeedsHandshake -> {
                        if (prep == com.meshchat.android.mesh.PrivateMediaPreparation.NeedsHandshake) {
                            try { initiateNoiseHandshake(recipientPeerID) } catch (_: Exception) { }
                        }
                        delay(500)
                    }
